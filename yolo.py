...
# Static variables
FILES_ID = [
    "16mQThTqt55T4gyYcM1jFSazX1rrgZvRC",
    "1UExXydQ_eES8PUOUY851J1bGpzSHNSOG",
    "1_zHnhIcm5NXxFV4kAqUDA6BHr97FoJdv",
]
    ...
    for i in range(len(FILE_NAMES)):
        if not Path(PATH_TO_STORE_MODEL + FILE_NAMES[i]).exists():
            ...
            download_file_from_google_drive(
                FILES_ID[i], PATH_TO_STORE_MODEL + FILE_NAMES[i]
            )
...
class _YOLO(object):
    ...
    def _boxes_and_scores(self, feats, anchors, num_classes, input_shape, image_shape):
        ...
        box_xy, box_wh, box_confidence, box_class_probs = self._yolo_head(
